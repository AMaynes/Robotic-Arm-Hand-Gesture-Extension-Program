...
class UserInterfaceApp:
    ...
    def check_and_start_tracking(self, arm_type, effector):
        ...
        """
        if not arm_type or not effector:
            messagebox.showwarning("Incomplete Selection", "Please select both Arm Type and End Effector!")
            return

        try:
            # Initialize the robotic arm based on the selected arm type
            robotic_arm = self.initialize_robotic_arm(arm_type)

            # Attempt to connect to the robotic arm
            if robotic_arm.connect(arm_config["port"], arm_config["baudrate"]):
                print("Connection successful.")
                beginTracking(arm_type)  # Start tracking hand gestures if connected
            else:
    ...
    def initialize_robotic_arm(self, arm_type):
        ...
        """
        if arm_type == "Dobot":
            return DobotArm()  # Return an instance of the DobotArm class
        elif arm_type == "uArm":
